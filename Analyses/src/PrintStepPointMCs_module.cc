...
//
// $Id: PrintStepPointMCs_module.cc,v 1.2 2013/10/21 20:44:04 genser Exp $
// $Author: genser $
...
namespace mu2e {
	...
	class PrintStepPointMCs : public art::EDAnalyzer {
		typedef SimParticleCollection::key_type key_type;
    typedef std::vector<int> Vint;
		public:

		explicit PrintStepPointMCs(const fhicl::ParameterSet&);
  ...
  PrintStepPointMCs::PrintStepPointMCs(const fhicl::ParameterSet& pset) : 
    art::EDAnalyzer(pset),
    ...
    instanceName_(pset.get<std::string>("inputInstanceName","virtualdetector"))
  {
	...
	void PrintStepPointMCs::analyze(const art::Event& event) {
		...
		if(hits.isValid())for(size_t k=0;k<hits->size();++k){
			...
			if (haveSimPart){
				if( !simParticles->has(trackId_hit) ){
					...
					key_type trackId_mo=trackId;
					while(hasPar&&print_parent){
						SimParticle const& sim = simParticles->at(trackId);
						SimParticle const& sim1 = simParticles->at(trackId_mo);//direct parent
						...
						else std::cout<<"PrintSP"<<id<<": "<<ii++<<" id: "<<sim.id()<<" pdgid: "<<sim.pdgId()<<" code: "<<sim.creationCode()<<" mom: "<<sim.startMomentum()<<" posh: "<<pos<<" momh: "<<mom<<" pos: "<<sim.startPosition()<<" pose: "<<sim1.endPosition()<<" mome: "<<sim1.endMomentum()<<" Time: "<<sim.startGlobalTime()<<" hitime: "<<hit.time()<<std::endl;
						hasPar=false;
						if(sim.hasParent()){
							hasPar=true;
							trackId=sim.parent()->id();
...
//using mu2e::PrintStepPointMCs;
DEFINE_ART_MODULE(mu2e::PrintStepPointMCs);
