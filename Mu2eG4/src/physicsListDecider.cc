...
#include "Mu2eG4/inc/Shielding_Wang.hh"
#if G4VERSION<4099
  ...
  G4VUserPhysicsList* physicsListDecider (const Config& config){
    ...
    if ( name  == "Minimal" ) {
      ...
      cout << "Warning: This Mu2e Physics List has not been certified for use with Geant4 v10+." << endl;
		...
		//>>Yaqian Wang
    else if ( name == "Shielding_Wang" ){
      tmpPL = new Shielding_Wang();
#if G4VERSION>4099
      cout << "Warning: This Mu2e Physics List with Geant4 v10+ where Atomic Deexcitation is enabled by Yaqian Wang." << endl;
#endif
		}
		//<<
